...
export default class QuestionSolver {
  constructor(page) {
    this.page = page;
    this.leetcodeDataDirectory = LEETCODE_DATA_PATH;
    ...
    try {
      try {
        ...
        if (acceptedText.includes("Solved")) {
          Logger.error(`${problemName} is already SOLVED!`);
      ...
      if (solutionAcceptedText === 'Accepted') {
        Logger.success(`${problemName} ACCEPTED`);
      } else {
        Logger.error(`${problemName} ${solutionAcceptedText}. Looks like the solution is old, contact the developer to fix this.`);
      }
      await sleep(1);
      await this.setAProblemSolved(problemName);
    } catch (err) {
      Logger.error(`Failed to solved the question ${problemName}`, err);
    }
  }

  async solveAllProblems() {
    for (let i = 0; i < this.allProblemNames.length; i++) {
      ...
      if (!checkIfSolved) {
        ...
        Logger.success(`${this.allProblemNames[i]} was solved earlier by the bot`);
      }
    }
  }
